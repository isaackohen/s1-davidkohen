        ...
        <div class="profile-container" v-if="profile">
            ...
            <div class="profile-content">
                ...
                <div class="tab_content" v-else-if="tab === 'info'">
                        ...
                        <div class="col-sm-12 col-md-6 col-xl-4 pl-0 pr-0" v-for="currency in currencies" v-if="currency.name">
                            <div class="user-profit">
                                ...
                                <div :class="`profit ${profile.profit[currency.id] < 0 ? 'text-danger' : (profile.profit[currency.id] === 0 ? '' : 'text-success')}`">
                                    {{ usd ? '$ ' + (profile.profit[currency.id] * currencies[currency.id].price).toFixed(2) : rawBitcoin(currency.id, profile.profit[currency.id]) }}
                                </div>
					...
					<div class="cat">{{ $t('general.profile.statistics.title') }}</div>
                    <table class="live-table">
                        <thead>
                            <tr>
                                <th>{{ $t('general.profile.statistics.currency') }}</th>
                                <th class="d-none d-md-table-cell">{{ $t('general.profile.statistics.wins') }}</th>
                                <th class="d-none d-md-table-cell">{{ $t('general.profile.statistics.bets') }}</th>
                                <th class="d-none d-md-table-cell">{{ $t('general.profile.statistics.wagered') }}</th>
                                <th>{{ $t('general.profile.statistics.wagered_usd') }}</th>
                            </tr>
                        </thead>
                        <tbody class="live_games">
                            <tr v-for="currency in currencies" v-if="currency.name && userStatistics != null">
                                <th>
                                    <div>
										<icon :icon="currency.icon" :style="{ color: currency.style }"></icon> <span>{{ currency.name }}</span>
                                    </div>
                                </th>
                                <th class="d-none d-md-table-cell">
                                    <div>
                                       {{ userStatistics[currency.id].wins }}
                                    </div>
                                </th>
                                <th data-highlight class="d-none d-md-table-cell">
                                    <div>
                                        {{ userStatistics[currency.id].bets }}
                                    </div>
                                </th>
                                <th data-highlight class="d-none d-md-table-cell">
                                    <div>
                                        {{ userStatistics[currency.id].wagered }} <icon :icon="currency.icon" :style="{ color: currency.style }"></icon>
                                    </div>
                                </th>
                                <th>
                                    <div>
                                       {{ userStatistics[currency.id].wagered_usd }} <i class="fas fa-usd-circle" style="color:#02b320" aria-hidden="true"></i>
                                    </div>
                                </th>
                            </tr>
                        </tbody>
                    ...
                    <table class="live-table">
                        ...
                        <tbody class="live_games">
                            <tr v-for="game in userGames">
                                ...
                                <th data-highlight class="d-none d-md-table-cell">
                                    <div>
                                        {{ usd ?  '$ ' + (game.game.wager * currencies[game.game.currency].price).toFixed(2) : rawBitcoin(game.game.currency, game.game.wager) }}
                                ...
                                <th>
                                    <div :class="game.game.status === 'win' ? 'live-win' : ''">
                                        {{ usd ? '$ ' + (game.game.profit * currencies[game.game.currency].price).toFixed(2) : rawBitcoin(game.game.currency, game.game.profit) }}
                                        <icon :icon="currencies[game.game.currency].icon" :style="{ color: currencies[game.game.currency].style }"></icon>
                                    </div>
                                </th>
                            </tr>
                ...
                <div class="tab_content" v-if="profile.isOwner && tab === 'settings'">
                    ...
                    -->
                    <div class="link-group">
                        {{ $t('general.profile.telegram') }}
                        <span>
                            <template v-if="profile.user.telegram">
                                <i class="fal fa-check mr-1"></i> {{ $t('general.profile.linked') }}
                            </template>
                              <vue-telegram-login 
								mode="redirect"
								telegram-login="davidkohen_bot"
								:redirect-url="'https://s1.davidkohen.com/api/callback/telegram/' + profile.user._id" v-else />
                        </span>
                    </div>

                    <div class="settingsNotify mt-2" v-if="profile.user.vipLevel > 0">
                        <span v-if="!profile.user.telegram" v-html="$t('general.profile.link_telegram')"></span>
                        <span v-else v-html="$t('general.profile.telegram_vip')"></span>
                        <button class="btn btn-block btn-primary mt-2">{{ $t('general.profile.telegram_vip_ok') }}</button>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['isGuest', 'currencies', 'games', 'user', 'theme', 'usd'])
        },
        created() {
        ...
        methods: {
            ...
            loadGames() {
                if(this.tab === 'info' && this.profile) axios.get(`/api/user/games/${this.profile.user._id}`).then(({ data }) => {
                    this.userGames = data;
                });
            },
			loadStatistics() {
                if(this.tab === 'info' && this.profile) axios.get(`/api/user/statistics/${this.profile.user._id}`).then(({ data }) => {
