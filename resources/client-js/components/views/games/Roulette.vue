<template>
    <div>
        ...
        <div class="roulette-field">
            <div class="header">
                {{ $t('general.wager') }}: <span style="margin-right:2px;" v-if="usd">$</span> <span id="bet">{{ usd ? (totalBet * currencies[currency].price).toFixed(2) : totalBet.toFixed(usd ? 2 : (currency.startsWith('local_') ? 2 : 8)) }}</span>
                <div class="right">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['quick', 'currency', 'usd', 'currencies'])
        },
        methods: {
            clear() {
                this.bet = {};
