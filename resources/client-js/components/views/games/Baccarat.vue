<template>
    <div>
        ...
        <div class="baccarat-field">
            <div class="header">
                {{ $t('general.wager') }}: <span style="margin-right:2px;" v-if="usd">$</span> <span id="bet">{{ rawBitcoin(currency, 0) }}</span> <icon :icon="currencies[currency].icon" :style="{ color: currencies[currency].style }"></icon>
                <div class="right">
...
<script>
    ...
    export default {
        computed: {
            ...mapGetters(['currency', 'currencies', 'usd'])
        },
        data() {
            return {
                _id: null,
        ...
        mounted() {
			...
			if(this.currency.startsWith('local_') || this.usd) {
                this.chipDisplayValue = 1.00;
                this.chip = 1.00 / this.currencies[this.currency].price;
            }
        },
        methods: {
            gameDataRetrieved() {
            ...
            customWagerCalculation() {
                return this.usd ? parseFloat(parseFloat($('#bet').html()) / this.currencies[this.currency].price) : parseFloat(this.rawBitcoin(this.currency, parseFloat($('#bet').html())));
            },
            clear() {
                this.bet = {};
            ...
            setBet(chip = null, value = null) {
                ...
                $('#bet').html(this.usd ? (total * this.currencies[this.currency].price).toFixed(2) : this.rawBitcoin(this.currency, total));
            },
            getClientData() {
                return {
                    bet: this.bet
            ...
            addPlayer(data) {
                _.forEach(data.data.bet, (value, key) => {
                    ...
                    $(`[data-container="${key}"] .os-content`).append(`<div class="user" onclick="window.open('/profile/${data.user._id}', '_blank')">
                        ...
                        <div class="bet">
                            ${this.usd ? ('$ ' + (value * this.currencies[data.game.currency].price).toFixed(2)) : this.rawBitcoin(data.game.currency, value)} ${this.currencies[data.game.currency].name.toUpperCase()}
                        </div>
                    </div>`);
                });
            },
