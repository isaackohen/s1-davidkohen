    ...
    if (! is_null($request->code)) {
        ...
        if (isset($obj->access_token)) {
            ...
            if (auth('sanctum')->guest()) {
                ...
                if (is_null($user)) {
                    createUser(null, $info['response'][0]['first_name'].' '.$info['response'][0]['last_name'], null, $photo, ['vk' => $info['response'][0]['id']]);
                } else {
                    $user->update([
                        'login_ip' => User::getIp(),
                        'login_multiaccount_hash' => request()->hasCookie('s') ? request()->cookie('s') : null,
...
Route::middleware('auth:sanctum')->post('discord_bonus', function () {
    ...
    if (($r->message ?? null) != null) {
        return APIResponse::reject(1);
    }
    if (auth('sanctum')->user()->discord_bonus) {
...
Route::middleware('auth:sanctum')->post('/discord_role', function () {
    if (auth('sanctum')->user()->vipLevel() < 1) {
    ...
    if (! is_null($request->code)) {
        ...
        if (isset($obj->access_token)) {
            ...
            if (auth('sanctum')->guest()) {
                ...
                if (is_null($user)) {
                    createUser(null, $info->username, null, null, ['discord' => $info->id]);
                } else {
                    $user->update([
                         'login_ip' => User::getIp(),
                         'login_multiaccount_hash' => request()->hasCookie('s') ? request()->cookie('s') : null,
    ...
    if (! $openid->mode) {
        ...
        if ($openid->validate()) {
            ...
            if (auth('sanctum')->guest()) {
                ...
                if (is_null($user)) {
                    createUser(null, $response['players'][0]['personaname'], null, null, ['steam' => $response['players'][0]['steamid']]);
                } else {
                    $user->update([
                        'login_ip' => User::getIp(),
                        'login_multiaccount_hash' => request()->hasCookie('s') ? request()->cookie('s') : null,
        ...
        if (isset($obj->access_token)) {
            ...
            if (isset($userInfo['id'])) {
                ...
                if (auth('sanctum')->guest()) {
                    ...
                    if (is_null($user)) {
                        createUser(null, $userInfo['name'], null, $userInfo['picture'] ?? null, ['fb' => $user_id]);
                    } else {
                        $user->update([
                            'login_ip' => User::getIp(),
                            'login_multiaccount_hash' => request()->hasCookie('s') ? request()->cookie('s') : null,
        ...
        if (isset($obj->access_token)) {
            ...
            if (isset($userInfo['id'])) {
                ...
                if (auth('sanctum')->guest()) {
                    ...
                    if (is_null($user)) {
                        createUser(null, $userInfo['name'], null, $userInfo['avatar'] ?? null, ['google' => $user_id]);
                    } else {
                        $user->update([
                            'login_ip' => User::getIp(),
                            'login_multiaccount_hash' => request()->hasCookie('s') ? request()->cookie('s') : null,
