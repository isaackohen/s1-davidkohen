...
class Roulette extends QuickGame implements MultiplierCanBeLimited
{
        ...
        return new SuccessfulQuickGameResult((new ProvablyFair($this))->result(), function (SuccessfulQuickGameResult $result, array $transformedResults) use ($user, $data, $totalBet) {
            ...
            if ($multiplier > 0) {
                ...
                foreach ((array) $data->game()->bet as $key => $value) {
                    ...
                    if (is_numeric($key) && intval($key) >= 0 && intval($key) <= 36 && $number == intval($key)) {
                        $profit += $value * HouseEdgeModule::apply($this, 36);
                    } elseif ($key === 'row1' && in_array(strval($number), $this->rows['first'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 3);
                    } elseif ($key === 'row2' && in_array(strval($number), $this->rows['second'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 3);
                    } elseif ($key === 'row3' && in_array(strval($number), $this->rows['third'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 3);
                    } elseif ($key === '1-12' && in_array(strval($number), $this->rows['numeric']['first'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 3);
                    } elseif ($key === '13-24' && in_array(strval($number), $this->rows['numeric']['second'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 3);
                    } elseif ($key === '25-36' && in_array(strval($number), $this->rows['numeric']['third'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 3);
                    } elseif ($key === 'red' && in_array(strval($number), $this->rows['red'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 2);
                    } elseif ($key === 'black' && in_array(strval($number), $this->rows['black'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 2);
                    } elseif ($key === 'even' && in_array(strval($number), $this->rows['e/o']['even'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 2);
                    } elseif ($key === 'odd' && in_array(strval($number), $this->rows['e/o']['odd'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 2);
                    } elseif ($key === '1-18' && in_array(strval($number), $this->rows['half']['first'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 2);
                    } elseif ($key === '19-36' && in_array(strval($number), $this->rows['half']['second'])) {
                        $profit += $value * HouseEdgeModule::apply($this, 2);
                    }
                }
    ...
    private function getMultiplier(array $result, Data $data)
    {
        ...
        foreach ((array) $data->game()->bet as $key => $value) {
            ...
            if (is_numeric($key) && intval($key) >= 0 && intval($key) <= 36 && $number == intval($key)) {
                $multiplier += HouseEdgeModule::apply($this, 36);
            } elseif ($key === 'row1' && in_array(strval($number), $this->rows['first'])) {
                $multiplier += HouseEdgeModule::apply($this, 3);
            } elseif ($key === 'row2' && in_array(strval($number), $this->rows['second'])) {
                $multiplier += HouseEdgeModule::apply($this, 3);
            } elseif ($key === 'row3' && in_array(strval($number), $this->rows['third'])) {
                $multiplier += HouseEdgeModule::apply($this, 3);
