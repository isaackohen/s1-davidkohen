...
class SuccessfulQuickGameResult implements GameResult
{
    ...
    public function winCustom(?User $user, Data $data, float $profit, float $displayMultiplier)
    {
        ...
        if (! $data->guest() && $profit > 0) {
            $user->balance(Currency::find($data->currency()))->quiet()->demo($data->demo())->add($profit, Transaction::builder()->game($data->id())->message('Win (Custom)')->get());
        }
    }
    ...
    public function win(?User $user, Data $data, float $multiplier, float $bet = null)
    {
        ...
        if ($multiplier < 1 && $bet <= Currency::find($data->currency())->minBet()) {
            $this->profit = 0;
        } else {
            $this->profit = ($bet * $multiplier);
        }

        if (! $data->guest() && $this->profit > 0) {
            $user->balance(Currency::find($data->currency()))->quiet()->demo($data->demo())->add($this->profit, Transaction::builder()->game($data->id())->message('Win')->get());
        }
    }
