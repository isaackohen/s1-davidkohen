...
{
    abstract public function start(\App\Game $game);

    abstract public function turn(\App\Game $game, array $turnData): Turn;

    abstract public function isLoss(ProvablyFairResult $result, \App\Game $game, array $turnData): bool;

    public function process(Data $data)
    {
        if (! $this->acceptsDemo() && ($data->guest() || $data->demo())) {
            return ['code' => -7, 'message' => 'This game does not accept any demo bets'];
        }
        if ($this instanceof MultiplayerGame && $this->state()->hasBetFrom($data->user()->_id)) {
            return ['code' => -6, 'message' => "Can't place more than one bet"];
        }
        if (! $this->acceptBet($data)) {
            return ['code' => -6, 'message' => 'Game is not accepting bets at this stage'];
        }

        if (! $data->guest()) {
            $data->user()->balance(Currency::find($data->currency()))->demo($data->demo())->subtract($data->bet(), Transaction::builder()->game($data->id())->message('Game')->get());
        }
    ...
    {
        if ($this instanceof MultiplayerGame && ! $this->allowCancellation()) {
            return;
        }
        ...
        $user = User::where('_id', $game->user)->first();
        if ($user != null) {
            $user->balance(Currency::find($game->currency))->demo($game->demo)->add($game->wager, Transaction::builder()->game($game->game)->message('Cancellation')->get());
        }
    }
