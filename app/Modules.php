...
class Modules extends Model
{
    ...
    public static function get(Games\Kernel\Game $game, bool $demo): ModuleData
    {
        if (self::where('game', $game->metadata()->id())->where('demo', $demo)->first() == null) {
            self::create([
        ...
        return new ModuleData($game, self::where('game', $game->metadata()->id())->where('demo', $demo)->first());
    }
}
...
class ModuleData
{
    ...
    public function __construct(Games\Kernel\Game $game, Modules $model)
    {
        $this->game = $game;
        $this->model = $model;
    ...
    public function activeModules($data = null, $pfResult = null, Game $game = null): array
    {
        $active = [];
        $result = [];
        foreach ($this->data as $module_id => $value) {
            array_push($active, $module_id);
        }

        foreach (Module::modules() as $module) {
            $instance = new $module($this->game, $game, $data, $pfResult);
            if (in_array($instance->id(), $active)) {
    ...
    public function toggleModule(Module $module)
    {
        if (! isset($this->data[$module->id()])) {
            ...
            foreach ($module->settings() as $setting) {
                $fill = array_merge($fill, [$setting->id() => $setting->defaultValue()]);
            }
