...
class Investment extends Model
{
    ...
    {
        if ($this->realShare != null && $this->realShare <= 0) {
            return 0;
        }
        if ($this->status == 1) {
            return $this->disinvest_share;
        }
    ...
    public static function getGlobalBankroll(Currency\Currency $currency)
    {
        $bankroll = 0;
        foreach (self::where('status', 0)->where('currency', $currency->id())->get() as $investment) {
            $bankroll += $investment->getProfit();
        }

        return $bankroll;
    }

    public static function getUserBankroll(Currency\Currency $currency, $user)
    {
        $bankroll = 0;
        foreach (self::where('status', 0)->where('user', $user->_id)->where('currency', $currency->id())->get() as $investment) {
            $bankroll += $investment->getProfit();
        }
    ...
    public static function getSiteProfitSince(Currency\Currency $currency, $created_at)
    {
    ...
    public static function getUserProfit(Currency\Currency $currency, $created_at, $user = null, $till_created_at = null)
    {
