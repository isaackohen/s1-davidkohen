...
class ChatController
{
    ...
    public function tip(Request $request)
    {
        ...
        if (floatval($request->amount) < floatval(auth('sanctum')->user()->clientCurrency()->option('min_tip')) || auth('sanctum')->user()->balance(auth('sanctum')->user()->clientCurrency())->get() < floatval($request->amount)) {
    ...
    public function rain(Request $request)
    {
        ...
        if (auth('sanctum')->user()->balance(auth('sanctum')->user()->clientCurrency())->get() < floatval($request->amount) || floatval($request->amount) < floatval(auth('sanctum')->user()->clientCurrency()->option('min_rain')) / 3) {
            return APIResponse::reject(2);
        }
